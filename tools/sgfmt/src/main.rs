...
mod config;
mod expressions;
use config::{resolve_options, Args, FormatOptions};
...
fn format_file(path: &Path, options: &FormatOptions) -> Result<String> {
    ...
    let program = SgParser::parse(&content).into_diagnostic()?;
...
mod tests {
    ...
    fn format_source(src: &str, options: FormatOptions) -> String {
        let program = SgParser::parse(src).expect("parse source");
        Formatter::new(options).format_program(&program)
    }
    ...
    fn formatter_is_idempotent() {
        ...
        let first = format_source(src, FormatOptions::default());
