...
### Requirement: Sengoo text SHALL use a pinned Unicode 17.0.0 UTF-8 foundation
...
### Requirement: The v0.2 stdlib profile SHALL include production HTTP evidence or residual

M3 SHALL consume archived `http-production-serving` evidence when available, or
SHALL leave that owner open with matrix residual rows (not Supported claims).
When archived, M3 SHALL
