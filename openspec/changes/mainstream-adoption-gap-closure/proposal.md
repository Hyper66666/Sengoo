...
2. **Cancellation semantics are deferred everywhere.** Three SUPPORT_MATRIX
   rows — task cancellation boundaries, select loser cancellation, and
   process cancellation — are `Deferred` with no active owning change after
...
### Pillar 1 — Source-level debugging (`native-debug-info`)
...
### Pillar 2 — Cancellation semantics (`async-cancellation-semantics`)
...
### Pillar 3 — Production HTTP serving (`http-production-serving`)
...
### Pillar 4 — Toolchain distribution (`toolchain-distribution`)
