...
- [ ] 1.1 Confirm coordinator entry gate and native ABI version.
- [ ] 1.2 Implement LLVM-target and direct-emitter spikes for the representative
  corpus; record artifacts, diagnostics, compile time, and maintenance cost.
...
- [ ] 4.1 Add `sgc build --target wasm` and explicit `sgc run --target wasm`
  runtime selection.
