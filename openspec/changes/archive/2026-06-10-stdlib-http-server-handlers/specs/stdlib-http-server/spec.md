...
`std::http` SHALL provide `server_next_request(handle, timeout_ms)` returning a
generation-checked request handle for the next inbound HTTP/1.1 request that is
not consumed by middleware, static routes, or ws-echo routes. The call SHALL
block at most `timeout_ms` milliseconds and SHALL map expiry to
`STATUS_TIMEOUT` without tearing down the server.

#### Scenario: A dynamic request is pulled

- **WHEN** a client sends `GET /compute` to a bound server with no matching static route
- **THEN** `server_next_request` returns a request handle for that request
- **AND** the server handle remains valid for further pulls

#### Scenario: No request arrives before the timeout

- **WHEN** `server_next_request` is called with `timeout_ms = 50` and no client connects
...
- **AND** `server_next_request` never observes that request
...
- **WHEN** a program built against the C-only bundle calls `server_next_request`
...
`sglsp` SHALL expose completions/signatures for the new `std::http` server
