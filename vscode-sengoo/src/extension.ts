...
let client: LspClientLike | undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('sengoo.showInfo', () => {
            vscode.window.showInformationMessage(
                'Sengoo Language v1.0.0 - Python 风格 + Rust 类型系统'
            );
        }),
        vscode.commands.registerCommand('sengoo.run', () => runSengooFile('run')),
        vscode.commands.registerCommand('sengoo.build', () => runSengooFile('build')),
        vscode.commands.registerCommand('sengoo.buildAndRun', () => runSengooFile('buildAndRun')),
        vscode.commands.registerCommand('sengoo.check', () => runSengooFile('check')),
    );

    // 注册调试配置提供者 (Initial - 处理 F5 无 launch.json 的情况)
    context.subscriptions.push(
        vscode.debug.registerDebugConfigurationProvider('sengoo',
            new SengooDebugConfigProviderInitial(),
    ...
    context.subscriptions.push(
        vscode.debug.registerDebugConfigurationProvider('sengoo',
            new SengooDebugConfigProviderDynamic(),
    ...
    context.subscriptions.push(
        vscode.debug.registerDebugConfigurationProvider('sengoo',
            new SengooDebugConfigResolver()
        )
    );

    // 注册调试适配器工厂
    context.subscriptions.push(
        vscode.debug.registerDebugAdapterDescriptorFactory('sengoo',
            new SengooDebugAdapterFactory()
        )
    );
    ...
    vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration('sengoo.lsp')) {
            ...
            const newEnabled = newConfig.get<boolean>('lsp.enabled', true);
...
function resolveWorkspaceRoot(filePath?: string): string | undefined {
    if (filePath) {
...
function getSgcPath(filePath?: string): string {
    ...
    if (configured) {
        ...
        if (!warnedInvalidConfiguredSgcPath) {
            ...
            vscode.window.showWarningMessage(
                `Sengoo 配置项 sengoo.sgc.path 无效：${configured}，将回退到自动探测。`
            );
        }
    }
    ...
    if (!warnedFallbackToPathSgc) {
        ...
        vscode.window.showWarningMessage(
            'Sengoo 未在项目内找到 target/debug/sgc，已回退到 PATH 中的 sgc（可能是旧版本）。'
        );
    }
    return 'sgc';
}
...
function runSengooFile(mode: 'run' | 'build' | 'buildAndRun' | 'check') {
    ...
    const cmd = (exe: string, ...args: string[]) => {
 ...
 * 提供默认配置列表让用户选择
 */
class SengooDebugConfigProviderInitial implements vscode.DebugConfigurationProvider {
 ...
 */
class SengooDebugConfigProviderDynamic implements vscode.DebugConfigurationProvider {
    provideDebugConfigurations(
        _folder: vscode.WorkspaceFolder | undefined,
        _token?: vscode.CancellationToken
    ): vscode.ProviderResult<vscode.DebugConfiguration[]> {
 ...
 * 我们需要在这里填充完整配置
 */
class SengooDebugConfigResolver implements vscode.DebugConfigurationProvider {
...
class SengooDebugAdapterFactory implements vscode.DebugAdapterDescriptorFactory {
    createDebugAdapterDescriptor(
        _session: vscode.DebugSession,
        _executable: vscode.DebugAdapterExecutable | undefined
    ): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
        return new vscode.DebugAdapterInlineImplementation(
            new SengooDebugSession()
        );
    }
}
 ...
 */
class SengooDebugSession implements vscode.DebugAdapter {
    ...
    private doLaunch(args: any) {
        const program: string = args.program || '';
        const sgcPath: string = args.sgcPath || 'sgc';
        const mode: string = args.mode || 'run';
        const cwd: string = args.cwd || path.dirname(program);
        const buildArgs: string[] = args.buildArgs || [];
    ...
    private doRun(sgcPath: string, program: string, cwd: string) {
    ...
    private doBuildAndRun(sgcPath: string, program: string, cwd: string, buildArgs: string[]) {
        ...
        buildProcess.on('close', (code: number | null) => {
            if (code === 0) {
                ...
                this.childProcess.on('close', (runCode: number | null) => {
    ...
    private sendOutputEvent(output: string, category: string) {
...
function startLspClient(
    ...
    client = new languageClientModule.LanguageClient(
        'sengoo-lsp',
        'Sengoo Language Server',
        serverOptions,
        clientOptions
    ) as LspClientLike;
