...
Date: 2026-07-15  
Decision: **promote the direct MIR-to-WASM scalar emitter** as the experimental
...
| --- | --- | --- | --- | --- |
| Direct MIR-to-WASM emitter in `tools/sgc/src/portable_backends.rs` | Valid core module + structural validator; Node/wasmtime execute `main` | No host C/clang; runs via Node or wasmtime | Ships inside `sgc` | One small emitter; ABI metadata custom section |
...
Promote the **direct emitter** for the experimental scalar v1 surface:
...
4. Unsupported ownership, memory, stdlib, and host-import surfaces remain
   fail-closed diagnostics instead of a partial runtime emulation layer.
...
This record selects the emitter only for the experimental scalar backend. It
does not claim completed WASI host imports, ownership/Drop lowering, memory or
output limit enforcement, or Windows plus Unix CI execution coverage.
