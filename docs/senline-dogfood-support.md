...
| installed-windows-x64 | package-smoke-proven | GitHub Actions run `29419695542` on `ba0d03ae3`: Windows package smoke green (archive with native runtime, install script/checksums, dual independent build compare, installed stdlib/run). Local dual target-dir builds produced identical `sgc.exe` and `sengoo_runtime.lib` under package-toolchain deterministic RUSTFLAGS. | Installed `senline-domain-worker` / HTTP product loops and reviewed Senline pin remain pending. |
| installed-linux-x64 | package-smoke-proven | Same run Ubuntu package smoke green (archive, install, dual A/B compare, installed stdlib/run). | Installed worker/HTTP product loops and reviewed Senline pin remain pending. |
