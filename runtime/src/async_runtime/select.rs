use super::bridge::cancel_scheduled_foreign;
use super::{
    clear_poll_wakeup_hint, merge_wakeup_hints, sengoo_async_cancel_dispatch,
    sengoo_async_drop_dispatch, sengoo_async_poll_dispatch, sengoo_async_result_dispatch_bool,
    sengoo_async_result_dispatch_f32, sengoo_async_result_dispatch_f64,
    sengoo_async_result_dispatch_i16, sengoo_async_result_dispatch_i32,
    sengoo_async_result_dispatch_i64, sengoo_async_result_dispatch_i8, take_poll_wakeup_hint,
    wait_for_wakeup_hint_or_yield,
};
...
unsafe fn cancel_select_losers(operands: &[(i64, i64)], winner: usize) {
    let winner_pair = operands[winner];
    let mut canceled_pairs = [(0_i64, 0_i64); MAX_SELECT_OPERANDS];
    let mut canceled_count = 0usize;

    for (index, (kind, handle)) in operands.iter().copied().enumerate() {
        if index == winner || handle == 0 || (kind, handle) == winner_pair {
            continue;
        }
        if canceled_pairs[..canceled_count].contains(&(kind, handle)) {
            continue;
        }
        if cancel_scheduled_foreign(kind, handle) {
            canceled_pairs[canceled_count] = (kind, handle);
            canceled_count += 1;
            continue;
        }
        if !sengoo_async_cancel_dispatch(kind, handle) {
            sengoo_async_drop_dispatch(kind, handle);
        }
        canceled_pairs[canceled_count] = (kind, handle);
        canceled_count += 1;
    }
...
#[no_mangle]
pub extern "C" fn sengoo_async_select_cancel_n_winner(
    count: i64,
    kind0: i64,
    handle0: i64,
    kind1: i64,
    ...
    unsafe {
        let winner = wait_for_first_ready_n(&pairs[..count]);
        cancel_select_losers(&pairs[..count], winner);
        winner as i64
    }
}
...
#[no_mangle]
pub extern "C" fn sengoo_async_select_cancel_winner(
    first_kind: i64,
    first_handle: i64,
    second_kind: i64,
    second_handle: i64,
) -> i64 {
    let pairs = [(first_kind, first_handle), (second_kind, second_handle)];
    unsafe {
        let winner = wait_for_first_ready_n(&pairs);
        cancel_select_losers(&pairs, winner);
        winner as i64
    }
}
