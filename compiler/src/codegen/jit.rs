//! LLVM JIT 浠ｇ爜鐢熸垚鍣?
//!
//! 浣跨敤 inkwell 鐢熸垚鐪熷疄鐨?LLVM IR 骞跺彲浠?JIT 鎵ц
...
pub struct JITCodegen {
    ...
    /// 涓存椂鍙橀噺璁℃暟鍣紙鐢ㄤ簬鐢熸垚鍞竴鍚嶇О锛?
    /// 鎵€鏈夊嚱鏁扮殑绛惧悕锛堢敤浜庣被鍨嬭浆鎹級
...
impl JITCodegen {
    ...
    fn codegen_function(&mut self, mir_fn: &MirFunction) -> Result<(), String> {
        ...
        // 鐢熸垚鍩烘湰鍧楋紙allocas 灏嗗湪绗竴涓熀鏈潡鍐呯敓鎴愶級
    ...
    fn codegen_basic_block(
        ...
        // 濡傛灉鏄涓€涓熀鏈潡锛屽湪杩欓噷鍒嗛厤灞€閮ㄥ彉閲忓苟瀛樺偍鍙傛暟
        if is_first {
        ...
        match inst {
            ...
            mir::Instruction::Load {
                ...
                match &src_ty {
                    ...
                    _ => {
                        // 鏅€氱被鍨嬶紝闇€瑕侀澶栫殑鎸囬拡灞傜骇锛坅lloca 杩斿洖鎸囬拡锛?
            ...
            mir::Instruction::Store { destination, value } => {
                ...
                match &dest_ty {
                    MIRType::Ptr(inner) => {
                        // destination 鏄?Ptr(T)锛岃〃绀烘垜浠兂瀛樺偍鍒版寚閽堟寚鍚戠殑浣嶇疆
                        let inner_ty = self.mir_type_to_llvm_str(inner);
            ...
            mir::Instruction::FieldAddr {
                ...
                // 缁撴瀯浣撳瓧娈靛湴鍧€璁＄畻: ptr = &base.field (瀛楁绱㈠紩鏄父閲?
            ...
            mir::Instruction::IndexAddr {
                ...
                match &base_ty {
                    MIRType::Array(_, _) => {
                        // 鏁扮粍绫诲瀷锛氶鍏堝皢鏁扮粍閫€鍖栦负鎸囧悜棣栧厓绱犵殑鎸囬拡锛圕 璇█琛屼负锛?
                        // base_ptr = &base[0]
                        self.emit_indent();
                        ...
                        // 灏嗗湴鍧€瀛樺偍鍒?destination local锛坉estination 鏄寚閽堢被鍨嬶級
                        // destination 鏄?Ptr(T)锛屽湪 LLVM 涓槸 T**
                    ...
                    MIRType::Tuple(field_tys) => {
                        // 鍏冪粍/缁撴瀯浣撶被鍨嬶細浣跨敤 getelementptr 璁块棶瀛楁
                        // 瀛楁绱㈠紩鏄父閲忥紝涓嶉渶瑕佸姩鎬佺储寮?
                        ...
                        // 鐩存帴浣跨敤 getelementptr 鑾峰彇瀛楁鍦板潃
            ...
            mir::Instruction::Call {
                ...
                if func == "print" || func == "println" {
                    ...
                    for (i, local) in args.iter().enumerate() {
                        ...
                        // 濡傛灉鐩爣鍑芥暟鍙傛暟绫诲瀷涓庡綋鍓嶇被鍨嬩笉鍚岋紝杩涜杞崲
    ...
    /// MIR 绫诲瀷杞?LLVM 绫诲瀷瀛楃涓?鈥?delegates to shared utility
