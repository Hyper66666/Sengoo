...
use super::common;
use crate::mir::{self, Local, LocalKind, MIRType, MirBinOp, MirConstant, MirFunction, MirUnOp};
...
impl JITCodegen {
    ...
    fn codegen_basic_block(
        ...
        // LLVM IR 基本块标签不需要 % 前缀，但引用时需要
        self.ir.push_str(&format!("bb_{}:\n", bb.id));
        self.indent += 1;

        // 如果是第一个基本块，在这里分配局部变量并存储参数
        if is_first {
            // 首先分配所有局部变量（包括 Temp、Param 和 Return 槽位）
    ...
    fn codegen_instruction(
        ...
        match inst {
            ...
            mir::Instruction::Load {
                ...
                // Load 指令：从 source 加载值到 destination
                let dest = self.local_name(*destination);
                let dest_ty = self.get_local_type(mir_fn, *destination);
                let src = self.local_reg(*source);
                let src_ty = self.get_local_type(mir_fn, *source);

                // 加载的值类型应该使用 destination 的类型
                let llvm_value_ty = self.mir_type_to_llvm_str(&dest_ty);

                // 对于 Ptr 类型的 source，需要双重加载
                match &src_ty {
                    MIRType::Ptr(inner) => {
                        // source 是 Ptr(T)，在 LLVM 中：
                        // - alloca 返回 T**
                        // - 第一次加载得到 T*
                        // - 第二次加载得到 T
            ...
            mir::Instruction::Unary {
                ...
                self.emit_indent();
                let operand_name = self.local_name(*operand);
                let dest = self.local_name(*destination);
                let ty = self.get_local_type(mir_fn, *destination);
                let llvm_ty = self.mir_type_to_llvm_str(&ty);
            ...
            mir::Instruction::IndexAddr {
                ...
                match &base_ty {
                    MIRType::Array(_, _) => {
                        ...
                        // addr_temp 是 T*，需要存储到 T**
                        self.emit_indent();
                        let dest_ptr_ptr_ty = format!("{}*", format!("{}*", llvm_elem_ty));
                        let addr_ptr_ty = format!("{}*", llvm_elem_ty); // addr_temp 的类型是 T*
                        self.ir.push_str(&format!(
                            "store {} {}, {} {}\n",
            ...
            mir::Instruction::Call {
                ...
                if func == "print" || func == "println" {
                    ...
                    self.emit_indent();
                    if return_ty != "void" {
                        // 使用临时名称作为 call 的结果，然后存储到 destination
        ...
        match terminator {
            mir::Terminator::Return(value) => {
                if let Some(local) = value {
                    ...
                    // 所有 local 都是通过 alloca 分配的，都需要加载
                    let reg = self.local_reg(*local);
    ...
    /// 二元操作转 LLVM 指令 — uses shared utility for opcode mapping
    ...
    fn get_type_size(&self, ty: &MIRType) -> u64 {
        match ty {
            ...
            MIRType::Ptr(_) | MIRType::Ref(_) => 8, // 指针大小
