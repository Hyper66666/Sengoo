...
pub struct Codegen {
    ...
    /// 棰勮绠楃殑 local 鍚嶇О缂撳瓨锛歭ocal.id 鈫?鍚嶇О瀛楃涓?
    pub(crate) name_cache: Vec<String>,
    /// MIR 绫诲瀷鍒?LLVM 绫诲瀷瀛楃涓茬殑缂撳瓨
...
impl Codegen {
    ...
    fn emit_header(&mut self) {
        // target triple 鏀惧湪 declarations 鏈€鍓嶉潰锛岀‘淇濆畠鏄?IR 鐨勭涓€琛?
        self.declarations.push_str("; Sengoo LLVM IR\n");
        self.declarations
            .push_str("; Generated by Sengoo Compiler\n\n");

        // 璁剧疆 target triple 閬垮厤 clang 鐨?-Woverride-module 璀﹀憡
    ...
    fn declare_runtime_functions(&mut self) {
        ...
        self.declarations.push_str("declare i8* @malloc(i64)\n");
        self.declarations.push_str("declare void @free(i8*)\n");
        self.declarations
    ...
    /// 鎵弿鎵€鏈?MIR 鍑芥暟锛屾敹闆嗙粨鏋勪綋绫诲瀷骞剁敓鎴?LLVM named struct type 澹版槑
    ...
    fn emit_module_ir(&mut self, mir_fns: &[MirFunction]) -> Result<(), String> {
        ...
        // 鎵弿鎵€鏈?MIR 鍑芥暟锛屾敹闆嗗苟鐢熸垚缁撴瀯浣撶被鍨嬪０鏄?
    ...
    fn codegen_instruction(
        ...
        match inst {
            ...
            mir::Instruction::Load {
                ...
                self.emit_indent();
                // 闇€瑕佸垽鏂槸鍚﹂渶瑕佺湡鐨?load锛?
                // 1. 鐢ㄦ埛鍙橀噺锛坅lloca锛夋€绘槸闇€瑕?load
                // 2. 鎸囬拡绫诲瀷鐨勪复鏃跺彉閲忎篃闇€瑕?load锛堜粠鍦板潃鍔犺浇鍊硷級
                ...
                if needs_load {
                    ...
                    // 涓存椂鍙橀噺宸茬粡鏄€硷紝鐩存帴澶嶅埗锛堜娇鐢?add 0, value 妯℃嫙 mov锛?
                    let src = self.local_name(*source);
                    self.ir
            ...
            mir::Instruction::Aggregate {
                ...
                match ty {
                    ...
                    MIRType::Struct { .. } => {
                        // 缁撴瀯浣撳垵濮嬪寲锛氫娇鐢?insertvalue 閾鹃€愬瓧娈垫瀯寤?
                        let llvm_ty = self.mir_type_to_llvm_cached(ty);
                        if fields.is_empty() {
                            self.emit_indent();
                            self.ir
            ...
            mir::Instruction::Call {
                ...
                // 澶勭悊鍙傛暟 鈥?鐢ㄦ埛鍙橀噺闇€瑕佸厛 load 鍐嶄紶閫?
                ...
                self.emit_indent();
                if is_print {
                    // print 鍑芥暟璋冪敤 puts锛岃繑鍥?i32锛屼絾鎴戜滑蹇界暐杩斿洖鍊?
                    self.ir
                        .push_str(&format!("call i32 @puts({})\n", arg_strs.join(", ")));
                    // print 杩斿洖 ()锛屾墍浠ユ垜浠渶瑕佽缃?dest 涓?unit 鍊?
                    self.ir.push_str(&format!("{} = add i8 0, 0\n", dest));
                } else if ret_ty == "void" {
                    self.ir.push_str(&format!(
                        "call void @{}({})\n",
            ...
            mir::Instruction::Discriminant {
                ...
                // 鑾峰彇鏋氫妇鐨勫垽鍒€?
                // 鍦?LLVM 涓紝鏋氫妇琛ㄧず涓?{ 鍒ゅ埆鍊? 杞借嵎 } 鐨勭粨鏋勪綋
                // 鍒ゅ埆鍊奸€氬父鏄涓€涓瓧娈碉紙绱㈠紩 0锛?
                let dest = self.local_name(*destination);
                let src = self.local_name(*source);

                // 浣跨敤 extractvalue 鎻愬彇鍒ゅ埆鍊?
                self.emit_indent();
                self.ir.push_str(&format!(
            ...
            mir::Instruction::Cast {
                ...
                self.emit_indent();
                match (&src_ty, to) {
                    // Int 鈫?Int: sext (sign-extend) for widening, trunc for narrowing
                    ...
                    // Float 鈫?Float: fpext for widening, fptrunc for narrowing
                    ...
                    // Int 鈫?Float: sitofp (signed int to floating point)
                    ...
                    // Float 鈫?Int: fptosi (floating point to signed int)
                    ...
                    // Bool 鈫?Int: zext (zero-extend i1 to iN)
                    ...
                    (MIRType::Ptr(_) | MIRType::Ref(_), MIRType::Int(_)) => {
                        self.ir.push_str(&format!(
                            "{} = ptrtoint {} {} to {}\n",
                            dest, src_llvm, src_val, dst_llvm
                        ));
                    }
                    (MIRType::Int(_), MIRType::Ptr(_) | MIRType::Ref(_)) => {
                        self.ir.push_str(&format!(
                            "{} = inttoptr {} {} to {}\n",
                            dest, src_llvm, src_val, dst_llvm
                        ));
        ...
        match terminator {
            mir::Terminator::Return(value) => {
                if let Some(v) = value {
                    ...
                    // 鐗规畩澶勭悊锛歮ain 鍑芥暟杩斿洖 unit 绫诲瀷鏃讹紝杩斿洖 0 鑰屼笉鏄?unit 鍊?
    ...
    /// 鍦?codegen_function 寮€濮嬫椂锛岄璁＄畻鎵€鏈?local 鐨勫悕绉?
    ...
    /// 甯︾紦瀛樼殑绫诲瀷杞崲锛氭鏌ョ紦瀛樺悗鍐嶈绠楋紝閬垮厤閲嶅杞崲鐩稿悓鐨?MIR 绫诲瀷
    ...
    fn get_local_type<'a>(&self, mir_fn: &'a MirFunction, local: Local) -> &'a MIRType {
        // 浣跨敤 id 鐩存帴绱㈠紩锛岃€屼笉鏄瘮杈?Local 瀵硅薄
        mir_fn
            .locals
            .get(local.index())
            .map(|(_, ty)| ty)
    ...
    /// 鑾峰彇鎿嶄綔鏁扮殑鍊煎瘎瀛樺櫒
    /// 濡傛灉鎿嶄綔鏁版槸鐢ㄦ埛鍙橀噺锛坅lloca锛夛紝鍒欑敓鎴?load 鎸囦护骞惰繑鍥炰复鏃跺瘎瀛樺櫒鍚?
    /// 鍚﹀垯鐩存帴杩斿洖瀵勫瓨鍣ㄥ悕
    fn operand_value(&mut self, local: Local, mir_fn: &MirFunction) -> String {
        // 浣跨敤 id 鐩存帴绱㈠紩锛孫(1) 鏌ユ壘 local 鐨勭被鍨嬩俊鎭?
        let local_info = &mir_fn.locals[local.index()].0;

        match local_info.kind {
            LocalKind::User => {
                // 鐢ㄦ埛鍙橀噺闇€瑕佸厛 load - 浣跨敤瀹為檯 MIR 绫诲瀷鑰岄潪纭紪鐮?i64
